...
class EditProfileStep2 extends StatefulWidget {
  const EditProfileStep2({super.key});

  @override
  State<EditProfileStep2> createState() => _EditProfileStep2State();
}

class _EditProfileStep2State extends State<EditProfileStep2> {
  ...
  Future handleCreateProfile() async {
    ...
    try {
      Response response = await clientApi.updateeMeProfile(
        profile.updateToJson(),
      );
  ...
  void initState() {
    ...
    _presentController.text = clientProvider.profile?.address.present ?? '';
    _permanentController.text = clientProvider.profile?.address.permanent ?? '';
